...
import org.example.solution.AlgorithmChallengeGraph.User;
...
public class AlgorithmChallengeGraph {
    ...
    static class SocialNetwork {
        ...
        public List<User> getFriends(User user) {
          return this.friendships.get(user); // aap user object ko as a KEY kese use kar sakte ho?? ek baar chatGPT ko poocho.
          // yha pe aapko equals() and hashCode() ka importance pta chalega :)
        }
        ...
        public List<User> getFriendSuggestions(User user) {
          // basically is problem me hame hamare friends k friends find karne hai ok..
          // ham -> hamare-friends -> unke-friends (ye wale log, jo 1st hand connection hai, LinkedIn pe bhi 1st 2nd asa kark aata hai)
          Set<User> suggestions = new HashSet<>(); // Set use kiya so that duplicate na ho.
          Set<User> directFriends = new HashSet<>(getFriends(user)); // ye hamare direct friends hai

          // basically hame jo hamare direct friends hai wo nahi chahiye
          directFriends.add(user);

          for (User friend: getFriends(user)) {
            for (User friendOfFriend : getFriends(friend)) {
        ...
        public List<User> findShortestPath(User start, User end) {
          // base case
          if (start.equals(end)) {
            return List.of(start); // List.of() ye immutable list deta hai, check karo ek baar ChatGPT se ki or kon kon se tarike se List crate kar sakte hain
          }
          // aap Queue<User> queue = new LinkedList<>(); se bhi queue create kar sakte ho, mujhe ArrayDeque sahi lagta hai :)
          ...
          while (!queue.isEmpty()) {
            ...
            for (User friendOfCurrent: getFriends(current)) {
              if (!visited.contains(friendOfCurrent)) {
                visited.add(friendOfCurrent);
                parentMap.put(friendOfCurrent, current);
                queue.offerLast(friendOfCurrent);

                // iska matlab, ham wo friend tak pohch gaye..
                if (friendOfCurrent.equals(end)) {
        ...
        public Map<Integer, List<User>> getUsersByDegree(User user, int maxDegree) {
          // ye wala thoda complex hai, isko saath me karenge, but try karna karne k liye isko
          // thoda time invest karo ye wale question pe, mja aayega:)
            throw new UnsupportedOperationException("Method not implemented yet");
        }
        
        private List<User> reconstructPath(Map<User, User> parent, User start, User end) {
            ...
            // reversing start se end karne k liye
